...
export const CreepyButton = ({
    ...
    return (
        <button
            className={cn(
                "relative min-w-[9em] rounded-xl bg-black outline-none select-none group tap-highlight-transparent",
                props.disabled ? "cursor-not-allowed opacity-75" : "cursor-pointer",
                "focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#f59e0b]",
                className
            )}
            onClick={onClick}
            onMouseMove={(e) => {
        ...
        >
            ...
            <motion.span
                ...
                animate={{
                    rotate: isHovered && !props.disabled ? -12 : 0,
                }}
                transition={{
                    type: "spring",
                    stiffness: 300,
